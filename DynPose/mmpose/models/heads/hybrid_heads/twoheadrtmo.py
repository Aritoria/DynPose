...
class RTMOHeadModule(BaseModule):
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                # print(x_net1[0].shape)
                x_net1 = self.net1.neck(x_net1)
                cls_scores, bbox_preds, kpt_offsets, kpt_vis, pose_feats = self.net1.head.head_module(x_net1)
            ...
            return cls_scores, bbox_preds, kpt_offsets, kpt_vis, pose_feats, \
                cls_scores_2, bbox_preds_2, kpt_offsets_2, kpt_vis_2, pose_feats_2, router_result
        else:
            if (self.net1_num + self.net2_num) % 100 == 0:
    ...
    def load_checkpoint(self,
        ...
        """
        checkpoint1 = _load_checkpoint('rtmo-l_16xb16-600e_body7-640x640-b37118ce_20231211.pth',
                                       map_location=map_location)
        checkpoint1 = _load_checkpoint_to_model(self.net2, checkpoint1, strict, revise_keys=revise_keys)
        print("Loaded RTMOl checkpoint")
        checkpoint2 = _load_checkpoint('rtmo-s_8xb32-600e_body7-640x640-dac2bf74_20231211.pth',
                                       map_location=map_location)
        checkpoint2 = _load_checkpoint_to_model(self.net1, checkpoint2, strict, revise_keys=revise_keys)
        print("Loaded RTMOs checkpoint")
...
class TwoHeadRTMO(YOLOXPoseHeadImage):
    ...
    def __init__(
        ...
        network2 = dict(
            ...
            backbone=dict(
                type='CSPDarknet',
                frozen_stages=4,
                deepen_factor=deepen_factor_2,
                widen_factor=widen_factor_2,
                out_indices=(2, 3, 4),
                spp_kernal_sizes=(5, 9, 13),
                norm_cfg=dict(type='BN', momentum=0.03, eps=0.001),
                act_cfg=dict(type='Swish'),
        ...
        self.head_module.net1 = MODELS.build(network1)
    ...
    def loss(self,
        ...
        flatten_cls_scores = self.head_module.net1.head._flatten_predictions(cls_scores)
        flatten_bbox_preds = self.head_module.net1.head._flatten_predictions(bbox_preds)
        flatten_objectness = torch.ones_like(
            flatten_cls_scores).detach().narrow(-1, 0, 1) * 1e4
        flatten_kpt_offsets = self.head_module.net1.head._flatten_predictions(kpt_offsets)
        flatten_kpt_vis = self.head_module.net1.head._flatten_predictions(kpt_vis)
        flatten_pose_vecs = self.head_module.net1.head._flatten_predictions(pose_vecs)
        flatten_bbox_decoded = self.head_module.net1.head.decode_bbox(flatten_bbox_preds,
                                                flatten_priors[..., :2],
                                                flatten_priors[..., -1])
        flatten_kpt_decoded = self.head_module.net1.head.decode_kpt_reg(flatten_kpt_offsets,
                                                  flatten_priors[..., :2],
                                                  flatten_priors[..., -1])
        ...
        targets = self.head_module.net1.head._get_targets(flatten_priors,
        ...
        if num_pos > 0:
            ...
            loss_box1 = self.loss_bbox(bbox_preds, bbox_targets) * 10 / num_total_samples
            loss_box2 = self.loss_bbox(bbox_preds_2, bbox_targets_2) * 10 / num_total_samples_2
            delt_box = loss_box1 - loss_box2
            # losses['loss_box1'] = torch.tensor(loss_box1, device=score.device)
            # losses['loss_box2'] = torch.tensor(loss_box2, device=score.device)
            # if torch.abs(delt_box) > 0.05:
            #     losses['loss_score1'] = (1 - score[0, 0]) * (loss_box1 - delt_box * 0.5) + score[0, 0] * (
            #             loss_box2 + delt_box * 0.5)
            #     if loss_box1 < 0.001:
            #         losses['loss_score2'] = torch.tensor(0.0, device=score.device)
            #     else:
            #         losses['loss_score2'] = ((delt_box / loss_box1) - score[0, 1]) * (
            #                 (delt_box / loss_box1) - score[0, 1])
            # else:
            #     losses['loss_score1'] = torch.tensor(0., device=score.device)
            #     losses['loss_score2'] = torch.tensor(0., device=score.device)
            #     # print("num_total_samples : ",num_total_samples)
            # losses['loss_score3'] = (score[0, 2] - num_total_samples / 100.0) * (
            #         score[0, 2] - num_total_samples / 100.0)
            if self.use_aux_loss:
                if hasattr(self, 'loss_bbox_aux'):
                    ...
                    # losses['loss_bbox_aux'] = (1-score[0,0])*(loss_bbox_aux_1 - delt_box_aux_1*0.5)+score[0,0]*(loss_bbox_aux_2 + delt_box_aux_1*0.5)
                    losses['loss_bbox_aux'] = torch.tensor(0.0, device=score.device) + 0*((1-score[0,0])*(loss_bbox_aux_1 - delt_box_aux_1*0.5)+score[0,0]*(loss_bbox_aux_2 + delt_box_aux_1*0.5))
            ...
            # losses['loss_vis'] = (1-score[0,0])*(loss_vis_1 - delt_vis*0.5)+score[0,0]*(loss_vis_2 + delt_vis*0.5)
            losses['loss_vis'] = torch.tensor(0.0, device=score.device) + 0*((1-score[0,0])*(loss_vis_1 - delt_vis*0.5)+score[0,0]*(loss_vis_2 + delt_vis*0.5))
            ...
            if hasattr(self, 'loss_mle') and self.loss_mle.loss_weight > 0:
                ...
                kpt_cc_preds, pred_hms, sigmas = self.head_module.net1.head.dcc.forward_train(pose_vecs,bbox_cs,pos_priors[..., :2])
                target_hms = self.head_module.net1.head.dcc.generate_target_heatmap(kpt_targets, bbox_cs, sigmas, pos_areas)
                loss_mle_1 = self.loss_mle(pred_hms, target_hms,vis_targets)
                loss_mle1 = torch.abs(loss_mle_1.sum()/loss_mle_1.size(0)) * 10
                # losses['loss_mle_1'] = loss_mle_1

                pose_vecs_2 = flatten_pose_vecs_2.view(-1, flatten_pose_vecs_2.size(-1))[pos_masks_2]
                bbox_cs_2 = torch.cat(bbox_xyxy2cs(bbox_preds_2, self.bbox_padding), dim=1)
                # 'cc' refers to 'cordinate classification'
                kpt_cc_preds_2, pred_hms_2, sigmas_2 = self.head_module.net2.head.dcc.forward_train(pose_vecs_2, bbox_cs_2,pos_priors_2[..., :2])
                target_hms_2 = self.head_module.net2.head.dcc.generate_target_heatmap(kpt_targets_2, bbox_cs_2, sigmas_2, pos_areas_2)
                loss_mle_2 = self.loss_mle(pred_hms_2, target_hms_2, vis_targets_2)
                # print("loss_mle_1",loss_mle_1.shape,"loss_mle_2",loss_mle_2.shape)
                loss_mle2 = torch.abs(loss_mle_2.sum()/loss_mle_2.size(0)) * 10
                # losses['loss_mle_2'] = loss_mle_2
                delt_mle = loss_mle2 - loss_mle1
                print("score : ", score, "loss_m1 :", loss_mle1, "loss_m2 :", loss_mle2)
                if torch.abs(delt_mle) > 0.05:
                    losses['loss_score1'] = (1 - score[0, 0]) * (loss_mle2 - delt_mle* 0.5) + score[0, 0] * (
                                loss_mle1 + delt_mle * 0.5)
                    if loss_mle1 < 0.001:
                        losses['loss_score2'] = torch.tensor(0.0, device=score.device)
                    else:
                        losses['loss_score2'] = ((delt_mle / loss_mle1) - score[0, 1]) * ((delt_mle / loss_mle1) - score[0, 1])
                else:
                    losses['loss_score1'] = torch.tensor(0., device=score.device)
                    losses['loss_score2'] = torch.tensor(0., device=score.device)
                # print("num_total_samples : ",num_total_samples)
            ...
            #     oks = self.head_module.net1.assigner.oks_calculator(kpt_cc_preds,
            #                                        kpt_targets,
        ...
        # losses['loss_cls'] = (1-score[0,0])*(loss_cls_1 - delt_cls*0.5)+score[0,0]*(loss_cls_2 + delt_cls*0.5)
        losses['loss_cls'] = torch.tensor(0.0, device=score.device) + 0 *((1-score[0,0])*(loss_cls_1 - delt_cls*0.5)+score[0,0]*(loss_cls_2 + delt_cls*0.5))
    ...
    def predict(self,
        ...
        # flatten predictions
        flatten_cls_scores = self.head_module.net1.head._flatten_predictions(cls_scores).sigmoid()
        flatten_bbox_preds = self.head_module.net1.head._flatten_predictions(bbox_preds)
        flatten_kpt_vis = self.head_module.net1.head._flatten_predictions(kpt_vis).sigmoid()
        flatten_pose_vecs = self.head_module.net1.head._flatten_predictions(pose_vecs)
        if flatten_pose_vecs is None:
            flatten_pose_vecs = [None] * len(batch_img_metas)
        flatten_bbox_preds = self.head_module.net1.head.decode_bbox(flatten_bbox_preds,
        ...
        for (bboxes, scores, kpt_vis, pose_vecs,
            ...
            if bboxes.numel() > 0:
                ...
                if flag == 1:
                    keypoints = self.head_module.net1.head.dcc.forward_test(pose_vecs, bbox_cs, grids)
                elif flag == 2:
