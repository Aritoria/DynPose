...
class TwoHeadRTMO(YOLOXPoseHeadImage):
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            losses['loss_score1'] = (1-score[0,0])*(loss_box1 - delt_box*0.5)+score[0,0]*(loss_box2 + delt_box*0.5)
            # print(self.loss_bbox(bbox_preds, bbox_targets) / num_total_samples)
            if loss_box1 < 0.001:
                losses['loss_score2'] = (score[0, 1]) * (score[0, 1])
            else:
                losses['loss_score2'] = ((delt_box / loss_box1) - score[0, 1]) * ((delt_box / loss_box1) - score[0, 1])
            losses['bbox_1'] = torch.tensor(1., device=score.device) * loss_box1
            losses['bbox_2'] = torch.tensor(1., device=score.device) * loss_box2

            losses['loss_score3'] = (score[0, 1] - num_total_samples/10.0) * (score[0, 1] - num_total_samples/10.0)
            ...
            #     kpt_cc_preds, pred_hms, sigmas = self.head_module.net1.dcc.forward_train(pose_vecs,bbox_cs,pos_priors[..., :2])
            #     target_hms = self.head_module.net1.dcc.generate_target_heatmap(kpt_targets, bbox_cs, sigmas, pos_areas)
