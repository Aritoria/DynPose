...
class DCC(BaseModule):
    ...
    def _pose_feats_to_heatmaps(self, pose_feats, x_bins_enc, y_bins_enc):
        ...
        # kpt_feats = self.pose_to_kpts(pose_feats)
        # print("kpt_feats", kpt_feats.shape)
...
class RTMOHead(YOLOXPoseHead):
    ...
    def loss(self,
        ...
        # print("flatten_priors: ", flatten_priors.shape)
        ...
        # print("pos_priors: ", pos_priors.shape)
        ...
        if num_pos > 0:
            ...
            if hasattr(self, 'loss_mle') and self.loss_mle.loss_weight > 0:
                ...
                # print('pose_vecs', pose_vecs.shape)
                # print('bbox_cs', bbox_cs.shape)
                # print('pos_priors', pos_priors.shape)
                kpt_cc_preds, pred_hms, sigmas = self.dcc.forward_train(pose_vecs,bbox_cs,pos_priors[..., :2])
                # print('kpt_cc_preds', kpt_cc_preds.shape)
                # print('sigmas', sigmas.shape)
                # print('pos_areas', pos_areas.shape)
                print("kpt_targets",kpt_targets.shape,kpt_targets)
                print("bbox_cs", bbox_cs.shape, bbox_cs)
                print("sigmas", sigmas.shape, sigmas)
                print("pos_areas", pos_areas.shape, pos_areas)
                target_hms = self.dcc.generate_target_heatmap(kpt_targets, bbox_cs, sigmas, pos_areas)
                # print('target_hms', target_hms)
                # print('pred_hms', pred_hms)
    ...
    def predict(self,
        ...
        flatten_pose_vecs = self._flatten_predictions(pose_vecs)
        if flatten_pose_vecs is None: flatten_pose_vecs = [None] * len(batch_img_metas)
