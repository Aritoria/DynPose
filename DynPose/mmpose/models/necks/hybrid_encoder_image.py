...
class HybridEncoderImage(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg: OptConfigType = dict(type='BN', requires_grad=True),
                 act_cfg: OptConfigType = dict(type='SiLU', inplace=True)):
        super(HybridEncoderImage, self).__init__()
    ...
    def forward(self, inputs_withimage: Tuple[Tensor]) -> Tuple[Tensor]:
        """Forward function."""
        # -1 是因为有图像
        with torch.no_grad():
            assert len(inputs_withimage)-1 == len(self.in_channels)
            image = inputs_withimage[-1]
            inputs = inputs_withimage[0:-1]
        ...
        newouts = outs + (image,)
        # print("neck_outs_image", image.shape)

        return tuple(newouts)
