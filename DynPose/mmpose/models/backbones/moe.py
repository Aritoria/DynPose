...
class MoE(BaseBackbone):
    ...
    def __init__(self):
        ...
        self.net1 = MODELS.build(network)
        self.net2 = MODELS.build(network)
        self.net3 = MODELS.build(network)
        self.net4 = MODELS.build(network)
        self.net5 = MODELS.build(network)
        ...
        self.layer2 = nn.Conv2d(256, 5, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        # only test
        max_idx = torch.argmax(score, dim=1, keepdim=True)  # 获取最大值的索引
        score = torch.zeros_like(score)  # 创建一个全零张量
        score.scatter_(1, max_idx, 1)  # 将最大值位置设为 1

        f1 = self.net1.backbone(image)
        f1 = self.net1.head(f1)
        f2 = self.net1.backbone(image)
        f2 = self.net1.head(f2)
        f3 = self.net1.backbone(image)
        f3 = self.net1.head(f3)
        f4 = self.net1.backbone(image)
        f4 = self.net1.head(f4)
        f5 = self.net1.backbone(image)
        f5 = self.net1.head(f5)

        # print(f5.shape)
        # print(score[:,0].view(-1,1,1,1).shape)

        f = score[:,0].view(-1,1,1,1)*f1 + score[:,1].view(-1,1,1,1)*f2 + score[:,2].view(-1,1,1,1)*f3 + score[:,3].view(-1,1,1,1)*f4 + score[:,4].view(-1,1,1,1)*f5
        outs.append(f)
